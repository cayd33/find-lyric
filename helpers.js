// Selectors
const findResultsContainer = $('#find-results-container');
const findResultsList = $('#find-results-list');

const lyricsContainer = $('#lyrics-container');
const lyricsResultsContainer = $('#lyrics-results');

const findLoader = $('#find-results-loader');
const lyricsLoader = $('#lyrics-loader');

// Helpers
function showfindLoader() {
    findLoader.show();
    findResultsList.hide();
}

function hidefindLoader() {
    findLoader.hide();
    findResultsList.show();
}
...
function showResultsContainer() {
    findResultsContainer.removeClass('d-none');
    findResultsContainer.addClass('d-flex');
}

function cleanupExistingResults() {
    findResultsList.html('');
...
function showFindError() {
    findResultsContainer.html('Sorry, there was an error with your request, please try again');
}
...
function scrollToFindResults() {
    $('html, body').animate(
        {
            scrollTop: findResultsContainer.offset().top - 60,
        },
        1500
    );
    hidefindLoader();
